...
onBeforeMount(() => {
    form.id_survey = props.id_survey;
});

function submit (){
    if(form.answers[1] != undefined){
        if(numberAnswer.number != form.answers.length){
            for(let i = numberAnswer.number+1; i < 20;i++ ){
                form.answers[i] = undefined;
            }
        }
    }
    router.post('/questions', form);
};
                                        ...
                                        <div class="flex flex-wrap -m-2">
                                            ...
                                            <label for="TypeQuestion" class="leading-7 text-sm text-gray-600">De quel type est votre question ?</label>
                                                ...
                                                <div class="p-2 w-1/3">
                                                    ...
                                                    <label for="Select" class="ml-2">Select</label>
                                                </div>
                                                <div class="p-2 w-1/3">
                                                ...
                                                <div class="flex flex-wrap">
                                                    <div class="relative">
                                                        ...
                                                        <input type="text" id="title" name="title" v-model="form.answers[answer]" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                                    </div>
                                                </div>
                                        </div>
                                    </div>
