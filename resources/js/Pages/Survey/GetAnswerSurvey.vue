...
onBeforeMount(() => {
    form.questions = props.survey['questions'].map(question => ({
        ...question,
        userAnswers: question.type === 'CheckBox' ? [] : null
    }));
});
                                        ...
                                        <div class="flex">
                                            <div class="p-2 w-1/3">
                                                <div class="relative">
                                                    ...
                                                    <div v-if="errors.lastName" class="text-red-600">Ce champ est obligatoire.</div>
                                                </div>
                                            </div>
                                            <div class="p-2 w-1/3">
                                                <div class="relative">
                                                    ...
                                                    <div v-if="errors.firstName" class="text-red-600">Ce champ est obligatoire.</div>
                                                </div>
                                            </div>
                                            <div class="p-2 w-1/3">
                                                <div class="relative">
                                                    ...
                                                    <div v-if="errors.city" class="text-red-600">Ce champ est obligatoire.</div>
                                                </div>
                                            </div>
                                        </div>
                                        <h1 class="mt-8 text-xl text-center font-medium title-font text-gray-900">Questions</h1>  
                                        <div v-for="(item, key) in survey.questions" :key="item.id">
                                            <div v-if="item.type == 'Text'" class="p-2 w-full">
                                                <div class="relative">
                                                    <label :for="'title-' + key" class="leading-7 text-sm text-gray-600">{{ item.content }}</label>
                                                    <input type="text" :id="'title-' + key" v-model="form.questions[key].userAnswers" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                                    <div v-if="errors[`questions.${key}.userAnswers`]" class="text-red-600">Ce champ est obligatoire.</div>
                                                </div>
                                            </div>
                                            <div v-if="item.type == 'Select'" class="p-2 w-full">
                                                <div class="relative">
                                                    <label :for="'answerSelect-' + key" class="leading-7 text-sm text-gray-600">{{ item.content }}</label>
                                                    <select :id="'answerSelect-' + key" v-model="form.questions[key].userAnswers" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                                        <option v-for="answer in item.answer" :key="answer.id" :value="answer.id">{{ answer.content }}</option>
                                                    </select>
                                                    <div v-if="errors[`questions.${key}.userAnswers`]" class="text-red-600">Ce champ est obligatoire.</div>
                                                </div>
                                            </div>
                                            <div v-if="item.type == 'CheckBox'" class="p-2 w-full">
                                                ...
                                                <div class="flex flex-wrap">
                                                    <div v-for="answer in item.answer" :key="answer.id" class="p-2 w-1/3">
                                                        <input type="checkbox" :id="'Checkbox-' + answer.id" v-model="form.questions[key].userAnswers" :value="answer.id" />
